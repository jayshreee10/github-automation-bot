...
describe('ActionRunner', () => {
  ...
  it('counts the attempt before acting, then records success with the result', async () => {
    ...
    expect(repo.succeed).toHaveBeenCalledWith('a1', { labels: ['bug'] }, expect.any(Number));
  });
  ...
  it('on retry, recovers a comment posted before a crash instead of posting twice', async () => {
    ...
    expect(repo.succeed).toHaveBeenCalledWith('a1', { id: 9, url: 'u9', recovered: true }, expect.any(Number));
  });
  ...
  it('a transient failure keeps the action pending and asks for a retry', async () => {
    ...
    expect(repo.fail).toHaveBeenCalledWith('a1', 'Slack webhook 503', false, expect.any(Number));
  });
  ...
  it('a permanent failure marks the action failed', async () => {
    ...
    expect(repo.fail).toHaveBeenCalledWith('a1', expect.stringContaining('404'), true, expect.any(Number));
  });
  ...
  it('labels and comments fail permanently on a push, which has no issue', async () => {
    ...
    expect(repo.fail).toHaveBeenCalledWith('a1', 'push has no issue or PR', true, expect.any(Number));
  });
});
