...
import type { GithubIdentityService } from '../../../modules/users/github-identity.service.js';
import type { InstallationsRepository } from '../../../modules/installations/installations.repository.js';
import { InstallationsService } from '../../../modules/installations/installations.service.js';
...
function setup() {
  ...
  const github = {
    getInstallation: vi.fn().mockResolvedValue({ id: 5, account: ACCOUNT, repository_selection: 'selected' }),
...
describe('InstallationsService', () => {
  describe('connect', () => {
    it('stores the installation and repos when GitHub says the caller owns it', async () => {
      ...
      expect(repo.saveWithRepos).toHaveBeenCalledWith(
        5,
        'user-1',
        { id: 5, account: ACCOUNT, repository_selection: 'selected' },
        REPOS,
      );
  ...
  it('list groups joined rows: ids as strings, rule counts, last events, and installations with no repos', async () => {
    const { service, repo } = setup();
    const alice = { installationId: 9007199254740993n, accountLogin: 'alice', accountType: 'User', repositorySelection: 'selected' };
    repo.listForUser.mockResolvedValue([
      { ...alice, repoId: 12n, fullName: 'alice/app', isPrivate: true, defaultBranch: 'main', ruleCount: 2, lastEventAt: new Date('2026-01-01T00:00:00Z') },
      { ...alice, repoId: 13n, fullName: 'alice/new', isPrivate: false, defaultBranch: null, ruleCount: 0, lastEventAt: null },
      {
        installationId: 6n,
        accountLogin: 'bob',
        accountType: null,
    ...
    await expect(service.list('user-1')).resolves.toEqual({
      installations: [
        { id: '9007199254740993', accountLogin: 'alice', accountType: 'User', repositorySelection: 'selected' },
        { id: '6', accountLogin: 'bob', accountType: null, repositorySelection: null },
      ],
      repositories: [
        {
          id: '12',
