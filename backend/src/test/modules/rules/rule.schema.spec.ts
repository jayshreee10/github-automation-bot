...
import { createRuleSchema, ruleDefinitionSchema, updateRuleSchema } from '../../../modules/rules/rule.schema.js';
...
describe('rule schemas', () => {
  ...
  it('defaults match to "all" for stored rules without it and validates excluded logins', () => {
    const r = ruleDefinitionSchema.parse({ ...base, conditions: { excludeAuthors: ['dependabot[bot]'] } });
    expect(r.conditions).toMatchObject({ match: 'all', excludeAuthors: ['dependabot[bot]'] });
    expect(messages({ ...base, conditions: { excludeAuthors: ['not a login'] } })).not.toEqual([]);
    expect(messages({ ...base, conditions: { match: 'some' } })).not.toEqual([]);
  });
