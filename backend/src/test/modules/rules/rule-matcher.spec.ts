...
import { matches } from '../../../modules/rules/rule-matcher.js';
import type { RuleConditions, RuleDefinition } from '../../../modules/rules/rule.schema.js';

const EMPTY: RuleConditions = {
  match: 'all',
  titleContains: [],
  bodyContains: [],
  authors: [],
...
describe('matches', () => {
  ...
  it('skips excluded authors, case-insensitively, even when everything else matches', () => {
    const r = rule({ titleContains: ['bug'], excludeAuthors: ['dependabot[bot]'] });
    expect(matches(r, repoEvent({ actor: 'Dependabot[bot]' }))).toBe(false);
    expect(matches(r, repoEvent())).toBe(true);
    expect(matches(rule({ match: 'any', excludeAuthors: ['alice'] }), repoEvent())).toBe(false);
  });

  it('with match "any", one non-empty condition is enough', () => {
    const r = rule({ match: 'any', titleContains: ['security'], bodyContains: ['security'] });
    expect(matches(r, repoEvent({ title: 'token in logs', body: 'a security leak' }))).toBe(true);
    expect(matches(r, repoEvent({ title: 'typo', body: 'nothing here' }))).toBe(false);
  });

  it('with match "any" and no conditions, matches every event of its type', () => {
    expect(matches(rule({ match: 'any' }), repoEvent())).toBe(true);
  });
