import { Bot, FolderGit2, LayoutDashboard, ListChecks, LogOut, ScrollText, TriangleAlert } from 'lucide-react'
...
export function Sidebar() {
  ...
  const nav = [
    ...
    { to: '/repositories', label: 'Repositories', icon: FolderGit2, badge: repos?.repositories.length, tone: 'muted' },
    { to: '/rules', label: 'Rules', icon: ListChecks },
    { to: '/failures', label: 'Failures', icon: TriangleAlert, badge: failures || undefined, tone: 'destructive' },
  ] as const
    ...
    <aside className="sidebar">
      <div className="brand">
        ...
        <span className="brand-text">
          ...
          <span className="brand-tagline">GitHub → Slack</span>
        </span>
      </div>
      ...
      <nav className="side-nav" aria-label="Main">
        ...
        {nav.map((item) => (
          <NavLink key={item.to} to={{ pathname: item.to, search }} end={'end' in item} className="side-link">
            <item.icon />
            {item.label}
...
function UserCard() {
  ...
  return (
    <div className="user-card">
      {me?.image ? <img className="avatar" src={me.image} alt="" /> : <span className="avatar">{name && initials(name)}</span>}
      <span className="user-text">
        <span className="user-name">{name}</span>
        <span className="user-handle">{me?.githubLogin ? `@${me.githubLogin}` : me?.email}</span>
      </span>
