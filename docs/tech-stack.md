...
- `src/components/ui/` is generated by shadcn (`npx shadcn add <name>`); treat as vendor code, don't hand-edit, excluded from lint. Style it from app code by passing semantic class names. One exception: `sonner.tsx` is pinned to the system theme so `next-themes` is not needed.
- Import alias `@/` → `src/`.
...
- Tests live in `src/test/`, mirroring `src/` by module (`src/test/features/auth/redirect.test.ts`), and import source via `@/`. Setup is `src/test/setup.ts`. Run with `npm test -w frontend`.
...
- Tests live in `src/test/`, mirroring `src/` by module (`src/test/modules/auth/auth.guard.spec.ts`), excluded from the build. Shared fakes and setup sit at `src/test/fakes.ts` and `src/test/setup.ts`. Run with `npm test -w backend`, or `npm test` at the root for both apps.
